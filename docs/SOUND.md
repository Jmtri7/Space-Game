...
| `explosion_small` | short (~0.15s), punchy boom — noise crackle, a falling-sweep thud, and a sine sub-thump, both held by a small `sustain` floor instead of decaying to a click (mixed quiet, `volume=0.4`) | **any weapon hitting an asteroid** — every hit, not just a destroying one, see `SpaceScreen._check_projectile_asteroid_collision` |
| `explosion_big` | a real ~1s boom — a sharp crack, a deep falling sweep, a held sub-thump, and a delayed second rolling low sweep for an aftershock, all four layers wider/lower/longer than `explosion_small`'s (`volume=0.7`) | **an asteroid actually breaking apart** — see `SpaceScreen._destroy_asteroid` |
