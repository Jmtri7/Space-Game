...
|---|---|---|
| `figures-human` | Just the human figure now: the walk rig, masc/femme bodies, generic wardrobe articles/items/sets, faces, palettes, base `materials.json`. `the_long_silence` keeps ~114 faction-specific files locally but **no kit shadows** — its 10 hand-tuned kit files (`human_femme`, `draw_order`, `eyes_almond` / `lips_full` / `nose_soft`, `badge` / `buttons` / `hair_short` / `pants` / `stand_collar`) replaced the module copies in `1.1.0`, so both consuming stories now share them. (The old module `human_femme`/`draw_order` carried extra leg-pants fitting curves/layers that nothing referenced; recover from git if ever needed.) Its settlement/scenery art moved to `orbital-std` in `2.0.0` — see that entry. | `the_long_silence`, `the_whisper_line`, `test_story` |
| `orbital-std` | Settlement/scenery art plus the **flat-catalogue** kind of sharing (`graphics.json`/`cultures.json`/`building_types.json`/`ship_types.json`, not the per-name `graphics/<kind>/<name>.json` kind `figures-human` provides for wardrobe) — the `orbital_std`/`regolith_std` cultures, the `courier` ship + `trade_ring` station (and their collision), `regolith_moon`, `admin_office`/`housing_block`/`market_hall` buildings, the shared furniture set (`bench`/`column`/`planter`/`lamp_post`/`crates`, decorations + collision), and two walkable interiors (`concourse`, `settlement`). This used to be bundled inside `figures-human` under a misleading name — it was pulled out in `2.0.0` since none of it is a human figure or wardrobe piece; any story using this scenery (including `the_long_silence`, for its five generic furniture building types) must now declare `orbital-std` explicitly rather than getting it for free from `figures-human`. | `the_whisper_line`, `mining_101`, `the_long_silence` |
| `audio-core` | The default `SoundBoard` recipes and the two ambient music loops, as data (`audio.json`). See [SOUND.md](SOUND.md). | all five stories |
| `ships-core` | The standard ship-equipment kit — `ship_outfits.json` (weapons/engines/utility) and `asteroid_types.json` — that `default` and `the_long_silence` had entry-for-entry identical. Ship *hulls* stay per-story (no two stories share one). | all five stories |
| `common-goods` | `items.json` — the personal inventory items every story carries (`repair_kit`, `medkit`, `star_chart`, `engraved_flask`). | all five stories |
| `story-defaults` | A partial `story.json`: camera + interior zoom bounds, `walking_speed`, the `jump` drive constants. Merged under each story's own `story.json`. | all five stories |
...
| `ls-authority`, `ls-combine`, `ls-drift`, `ls-vigil`, `ls-warden`, `ls-carrier` | One module per `the_long_silence` culture, each a complete culture kit: `graphics/{articles,buildings,palettes,sets,ships,stations,floor_patterns}/<pfx>_*` per-name files, plus that culture's slice of the flat catalogues (`graphics.json`'s `ships`/`space_stations`/`outfits`, `building_types.json`'s `<pfx>_hall`/`<pfx>_housing`/`<pfx>_spire`, `ship_types.json`'s `<pfx>_courier`/`<pfx>_hauler`/`<pfx>_patrol`). Split out (2026-09-12) from files that used to live directly under `config/stories/the_long_silence/`, so a future story wanting just one culture's look (as `the_whisper_line` already does — it depends on `ls-authority` alone for its single `floor_pattern: "authority"` station) can take it without the other five. Replaces the old `long-silence-floors` module, whose six `floor_patterns/<name>.json` files are now one per culture module instead of bundled separately. `cultures.json`/`factions.json`/`commodities.json`/moons and the narrative catalogues (`missions.json`, `dispatches.json`, `endings.json`) stayed in the story — not graphics, and not obviously reusable outside this story's plot. | `the_long_silence` (all six); `the_whisper_line` (`ls-authority` only) |
