...
(`game/graphics/expand.py`) → flat parts list, used identically by every story
that draws through it (at load) and by the vertex editor at
[`config/editor.html`](../config/editor.html). `default` does not run on this
pipeline — see above.
