...
y139-194, bare head (70,46) r14 with the mouth line at y54, helmeted face
(70,48) r11.5. The arms are splayed out from the body by `gen_si.ARM_REST_DEG`
about the shoulder joint, so anything drawn on an arm (a cuff, a glove, an
armband) is authored at its straight-down position and wrapped in
`_arm_rot(s, x, y)` (s = -1 left / +1 right). Anything on the hip/thigh that the
arm should cover goes in `pre` (drawn behind the body), not `post`.
"""
import math
import pathlib
import sys
