...
class Autopilot:
    ...
    def predict_trajectory(self, target, max_frames=500, sample_rate=5):
        """Forward-simulate seek-mode flight toward `target`, without touching real ship state.

        Returns list of (x, y) positions sampled every sample_rate frames - used
        to draw the predicted landing path/marker overlay.
        """
        ship = self.ship
        if not target:
            return []

        waypoints = [(ship.x, ship.y)]  # Start with current position
        sim_x, sim_y = ship.x, ship.y
        sim_vx, sim_vy = ship.velocity_x, ship.velocity_y
        sim_angle = ship.angle
        ...
        for frame in range(max_frames):
            ...
            if angle_diff < -ship.rotation_speed:
                sim_angle = (sim_angle - ship.rotation_speed) % 360
            elif angle_diff > ship.rotation_speed:
                sim_angle = (sim_angle + ship.rotation_speed) % 360

            aligned = abs(angle_diff) < 10
            sim_thrust = ship.acceleration_magnitude if aligned else 0
            ...
            if ship.space_drag > 0:
                sim_vx *= ship.space_drag
                sim_vy *= ship.space_drag
    ...
    def predict_position(self, target, max_frames=500):
        """Predict where the ship will end up given current seek-mode trajectory.

        Returns (final_x, final_y, distance_from_target).
        """
        ship = self.ship
        if not target:
            return ship.x, ship.y, 0

        waypoints = self.predict_trajectory(target, max_frames)
        if waypoints:
