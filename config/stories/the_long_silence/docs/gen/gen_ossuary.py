     ...
     "dialogue_tree": {"root": "start", "conditional_roots": [
      ...
      "nodes": {
        ...
        "done": {"text": "You have read what we keep, and I passed the same warning to you by dispatch besides - the dead get no other voice, so we use every one we have. You know the Silence was a choice, roughly what it cost, and that the Vigil would neither make it again the same way nor unmake it carelessly. The Span's beacon is lit. Go and choose.",
                 "options": [
                     {"label": "I have the reactivation front's timing. The Vigil should have it.", "next": "front_delivered",
                      "requires_flag": "front_recon_have_record", "requires_not_flag": "front_recon_delivered",
                      "actions": ["set_flag:front_recon_delivered", "set_flag:front_to_vigil",
                                  "adjust_rep:the_vigil:10", "adjust_rep:harbor_authority:-4"]},
                     {"label": "Pledge the Vigil your caution.", "next": "pledged",
                      "requires_rep": "the_vigil:25", "requires_not_flag": "patron:the_vigil",
                      "actions": ["set_exclusive_flag:patron:the_vigil", "adjust_rep:the_vigil:8"]},
                     {"label": "Understood", "next": None}]},
        "front_delivered": {"text": "So we know the hour the signal reaches each beacon. Good. We cannot stop it - but we can be standing at every lane it opens, with the wall's account in hand, before the Authority's flag gets there. You have bought the dead a hearing. The Authority will call that obstruction, and hold it against you. Let them.",
