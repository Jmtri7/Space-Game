     ...
     "dialogue_tree": {"root": "start", "conditional_roots": [
      ...
      "nodes": {
        ...
        "done": {"text": "The room chose - narrowly, and nobody's happy, which is how you know it was fair. Ossuary's lane is open, and we've sent word we may need friends soon. Thank you for making us sit down.",
                 "options": [
                     {"label": "I have the reactivation front's timing. The Drift should have it.", "next": "front_delivered",
                      "requires_flag": "front_recon_have_record", "requires_not_flag": "front_recon_delivered",
                      "actions": ["set_flag:front_recon_delivered", "set_flag:front_to_drift",
                                  "adjust_rep:the_drift:10", "adjust_rep:free_carrier:4"]},
                     {"label": "Pledge the Drift your voice.", "next": "pledged",
                      "requires_rep": "the_drift:25", "requires_not_flag": "patron:the_drift",
                      "actions": ["set_exclusive_flag:patron:the_drift", "adjust_rep:the_drift:8"]},
                     {"label": "Safe assembly", "next": None}]},
        "front_delivered": {"text": "Every beacon, to the minute - so we know exactly how long we have at each one. The assembly will still argue, but now it argues against a clock, and that it can do. This buys the outer lanes a running start. The carriers will hear you brought it to us; that's a debt in your favour at every slip in the region.",
